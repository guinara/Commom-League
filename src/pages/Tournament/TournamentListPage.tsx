 ...
 const { t, i18n } = useTranslation();
  ...
  const handleReady = async () => {
    try {
      ...
      toast.error('An error occurred while registering for the tournament. Please try again.');
    }
  };
                  ...
                  <Container>
                    {data.map((tournament) => (
                      <TournamentCard key={tournament.id} bgImage={`../../../../public/imagens/galeria/foto-5.png`}>
                        <StatusOverlay>
                        {t(tournament.status === 'TOURNAMENT_LOSE' ? 'Participated in the Tournament' : 'Lost the Tournament')}
                        </StatusOverlay>
                        <InfoContainer>
                          <InfoColumn>
                            <Info>
