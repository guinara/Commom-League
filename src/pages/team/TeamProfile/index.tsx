...
if (captain) {
  ...
  setCaptainId(captain.user.id); // Atualiza o ID do capitão
        ...
        setData(null)
        toast.error(t("You are not participating in any team."), {
  ...
  const handleSubmit = async (values: TeamFormValues) => {
    ...
    try {
      ...
      toast.success(t("Invitation sent successfully!"));  // Mensagem do toast
    } catch (error) {
      toast.error(t("Error sending the invitation."));
    }
  };
  ...
  const handleSelectCaptain = async (leaderId: string) => {
    ...
    try {
     ...
     const teamData = {
      ...
      toast.success(t('Captain changed successfully!'));
      setCaptainId(null);
      window.location.reload();
    } catch (err) {
      console.error(err);
      toast.error(t('Error changing captain!'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLeaveTeam = async () => {
    ...
    try {
      ...
      await teamService.leave(data.id);  
      toast.success(t('You successfully left the team!'));

      setData(null); 
    } catch (err) {
      console.error(err); 
      toast.error(t('Error leaving the team!')); 
      setError(t('Error trying to leave the team')); 
    } finally {
      setLoading(false); 
    }
  };
  ...
  const handleBan = async (userId: String) => {
    ...
    try {
      ...
      await teamService.ban(userId); 
      toast.success(t('You successfully banned the player!')); 
      window.location.reload();
    } catch (err) {
      console.error(err); 
      toast.error(t('Error banning the player!')); 
    } finally {
      setLoading(false); 
    }
  };
  ...
  const validationSchema = Yup.object().shape({
    players: Yup.array()
      .of(Yup.string().required(t('The players name is required.')))
      .test(
        'max-players',
  ...
  const handleInvite = (name: string) => {
   // console.log(t(`Invite player: ${name}`);
  };
        ...
        <Components.Banner className={`banner ${active ? 'active' : ''}`}>
          ...
          {data && data.name ? (
            <Components.UserBanner>
              ...
              <Components.InfoDiv>
                <Components.h2>{data?.name || 'Unavailable'}</Components.h2>
              </Components.InfoDiv>
  ...
  <div>{t("Nenhum jogador encontrado ou dados ainda carregando...")}</div>
)}
            </PlayerListContainer>
          )}
