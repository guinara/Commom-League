...
const handleWithdraw = async (values: SaleClientRequestDTO) => {
  const { qnt, password, chipId } = values; // Pega chipId diretamente de values
  ...
  if (qnt > saldoFicha!) {
    toast.error(t('You do not have enough chips for this withdrawal.'));
    setWithdrawLoading(false);
    return;
  }

  try {
    // Chama o serviço de venda de fichas com chipId vindo de values (selecionado pelo usuário)
    const response = await chipsService.sellChip({
      qnt,        // Quantidade de fichas
      password,   // Senha do usuário
      chipId,     // chipId vindo de values
    });
    ...
    toast.error(t('Error while making the withdrawal.'));
  } finally {
    setWithdrawLoading(false);
  }
};
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await chipsService.consult(); 
      } catch (error) {     
      }
    };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await trasactionService.consult(); 
        if (response.data && response.data.content.length > 0) {
          setTransacoes(response.data.content); 
          setSaldo(response.data.content[0]?.qnt || 0); 
        } else {
        }
      } catch (error) {
      }
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      //    console.log(`saldo ${response.data[0].qnt}`);
          setSaldoFicha(response.data[0].qnt); 
        } else {
   ...
   //   console.log(t("Notificação Aceita:"), notifications[index]);  // Verifique o conteúdo aqui
      ...
      try {
       ...
       // console.log("Enviando notificação:", notificationObj);
        ...
        toast.success(t("Notification accepted successfully!"));
      } catch (error) {
        toast.error(t("Error accepting the notification"));
      }
    };
    ...
    //  console.log("Notificação Recusada", notifications[index]);  // Verifique o conteúdo aqui
      ...
      try {
       ...
       // console.log("Enviando notificação:", notificationObj);
        ...
        toast.success(t("Notification rejected successfully!"));
      } catch (error) {
        toast.error(t("Error rejecting the notification"));
      }
    };
  ...
  // console.log("ola")
    ...
    toast.error(t('An error occurred while processing your payment. Please try again.'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {modalStep === 'addFunds' ? t('Add Chips') : t('Withdraw Chips')}
        </Typography>
        ...
        >
       {modalStep === 'addFunds' ? t('Go to Withdraw') : t('Go to Add Chips')}
        </Button>
  ...
  {saldo != null
    ? `${t('Available chips:')} ${saldoFicha} ${t('Chips')}`
    : t('Chips not available')}
</Typography>

                  <Typography variant="body2" sx={{ marginTop: 1 }}>
  {values.qnt != null
    ? `${t('Chips to add:')} ${values.qnt} ${t('Chips')}`
    : ''}
</Typography>
                ...
                <TextField
                  ...
                  id="qnt"
                  name="qnt"
                  placeholder={t("Digite a quantidade de fichas")}
                  fullWidth
                  sx={{ marginTop: 2 }}
                />
                <Button
          ...
          <Formik
          initialValues={{ qnt: 0, password: '', chipId: '67b84e5a-3d6a-4e69-9647-30ec76ae8143' }} // Valor fixo para chipId
          ...
          >
            {({ setFieldValue, values }) => (
              <FormikForm>
                <Box sx={{ marginTop: 2 }}>
                  <Typography variant="body1">
                  {t("Current chips:")} {saldoFicha !== null ? `${saldoFicha} Chips` : 'Loading...'}
                  </Typography>
                    ...
                    <>
                      ...
                      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, marginTop: 2 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                          ...
                          <Typography variant="body2">{saldoFicha} {t("Chips")}</Typography>
                        </Box>
                      </Box>
   ...
   {/* Select para CommonCoin */}
   <FormControl fullWidth sx={{ marginTop: 2 }}>
              <InputLabel>{t("CommonCoin")}</InputLabel>
              <Select
                label={t("CommonCoin")}
                value={values.chipId}  // Valor fixo para o select
                onChange={e => setFieldValue('chipId', e.target.value)} // Atualiza o chipId com o valor do Select
              >
                {/* Exemplo de opções no Select */}
                <MenuItem value="67b84e5a-3d6a-4e69-9647-30ec76ae8143">
                  {t("CommonCoin")} 
                </MenuItem>
                {/* Adicione mais opções conforme necessário */}
                      ...
                      <Button
                        ...
                        sx={{ marginTop: 2 }}
                      >
                       {isLoading ? t('Loading...') : t('WITHDRAW')}
                      </Button>
                    </>
                  ) : (
                    <Typography variant="body2">
                      {t('Insufficient balance for withdrawal.')}
                    </Typography>
                  )}
                </Box>
              </FormikForm>
          ...
          <Box>
          <Typography variant="body1">{t('Withdrawal successful!')}</Typography>
            <Button
              variant="contained"
        ...
        <Box sx={{ marginTop: 2 }}>
        <Typography variant="body2">{t('1 chip = 5 BRL')}</Typography>
        </Box>
      </Box>
    </Modal>
...
<Dialog open={inventoryModalOpen} onClose={closeInventoryModal} fullWidth>
        <DialogTitle>
        <InventoryIcon /> {t('Inventory')}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ width: '100%' }}>
            <Tabs value={selectedTab} onChange={handleTabChange} aria-label="inventory tabs">
            <Tab label={t('Chip Wallet')} />
            <Tab label={t('Transaction History')} />
            </Tabs>
            {selectedTab === 0 && (
              <Box sx={{ padding: 2 }}>
                {/* Seção da Carteira de Fichas */}
                <h3>{t('Amount of Chips')}</h3>
                <p>{t('You have')} {saldoFicha} {t('chips in your wallet.')}</p>
              </Box>
            )}
            {selectedTab === 1 && (
              <Box sx={{ padding: 2 }}>
...
<TableCell>{t('Amount of Chips')}</TableCell>
        ...
        <DialogActions>
          <Button onClick={closeInventoryModal} color="primary">{t("Close")}</Button>
        </DialogActions>
      </Dialog>
    </header>
  );
